...
import com.oryx.core.annotation.decl.Mandatory;
import com.oryx.core.annotation.decl.ParamBuilder;
import com.oryx.core.annotation.decl.Parameter;
...
@SupportedAnnotationTypes(ParamBuilderProcessor.AnnotationFqn)
@SupportedSourceVersion(SourceVersion.RELEASE_8)
public class ParamBuilderProcessor extends AbstractProcessor {
    public final static String AnnotationFqn = "com.oryx.core.annotation.decl.ParamBuilder";
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (Element e : roundEnv.getElementsAnnotatedWith(ParamBuilder.class)) {
    ...
    private void processExecutableElement(ExecutableElement element) {
        ...
        ParamBuilder paramBuilderAnnotation = element.getAnnotation(ParamBuilder.class);
        String finalMethodName = defaultFinalMethodName;
        if (!paramBuilderAnnotation.finalMethodName().isEmpty())
            finalMethodName = paramBuilderAnnotation.finalMethodName();
        String packageName = getPackageName(element);
        if (!paramBuilderAnnotation.builderPackage().isEmpty())
            packageName = paramBuilderAnnotation.builderPackage();
        String builderClassName = defaultBuilderClassName;
        if (!paramBuilderAnnotation.builderName().isEmpty())
            builderClassName = paramBuilderAnnotation.builderName();
